...
export const IcsEventButton = ({event}: IcsEventButtonProps) => {
    ...
    const getTimeDiffInHours = (startDate: number, endDate: number): 
      number => {
    ...
    const getTimeDiffInMinutes = (startDate: number, endDate: number):
      number => {
    ...
    return (
        <div>
            <button className="button ics-event-button" onClick={generateIcsEvent}>→ ICalendar</button>
        </div>
    );
};
